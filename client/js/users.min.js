 ...
 * License: GPLv3
 */
Vue.component("users-change-password",{template:'<div class="users change-password"><div v-if="!error.server" class="ds-container"><h1 v-if="page_data.password_expired">Password Expired</h1><h3>Update Account Password</h3><aui-input type="password" label="Password" :failure="error.password" v-model="form.password_1"/><aui-input type="password" label="Confirm Password" :failure="error.password" v-model="form.password_2" /><aui-button @click="changePassword" :loading="state.calling" loading_text="Updating...">Update</aui-button></div><aui-notice v-else type="failure" icon="fas fa-exclamation-triangle">Server Error</aui-notice></div>',data:()=>({state:{calling:!1},form:{password_1:null,password_2:null},error:{server:!1,password:null},page_data:{password_expired:!1}}),methods:{changePassword(){this.state.calling=!0,this.error.password=null;let a=new URLSearchParams(window.location.search.substr(1)).get("ref");null!==a&&""!==a||(a="/login"),DynamicSuite.call("users","change.password",this.form,s=>{switch(s.status){case"OK":window.location=a;break;case"BAD_PASSWORD":case"NO_CHANGE_REQUIRED":this.error.password=s.message,this.state.calling=!1;break;default:this.error.server=!0}})}},mounted(){this.page_data=DynamicSuite.getPageData()}});
